...
def test_cli_nfe_arquivo_inexistente(tmp_path):
    arquivo_inexistente = tmp_path / "nao_existe.xml"
    base_inexistente = tmp_path / "base_nao_existe.xlsx"

    result = runner.invoke(app, [
        "nfe",
        "-n", "teste_erro_nfe",
        "-x", str(arquivo_inexistente),
        "-b", str(base_inexistente)
    ])

    assert result.exit_code == 1
    assert "Erro: O arquivo XML" in result.output
